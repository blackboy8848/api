/**
 * Categories API (get one, update, delete).
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    if (Array.isArray(list) && list.length === 0) {
      const res = NextResponse.json({ error: 'Category not found' }, { status: 404 });
      return addCorsHeaders(res, origin);
    }
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const [result] = await db.execute(`UPDATE categories SET ${setClause} WHERE id = ?`, [...values, id]);
    ...
    const res = NextResponse.json({ message: 'Category updated successfully', id });
    return addCorsHeaders(res, origin);
  } catch (error: unknown) {
    const err = error as { message?: string };
    const res = NextResponse.json({ error: err?.message || 'Server error' }, { status: 500 });
...
// DELETE — Delete category (subcategories deleted via CASCADE)
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const res = NextResponse.json({ message: 'Category deleted successfully' });
    return addCorsHeaders(res, origin);
  } catch (error: unknown) {
    const err = error as { message?: string };
    const res = NextResponse.json({ error: err?.message || 'Server error' }, { status: 500 });
